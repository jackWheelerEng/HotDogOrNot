#Section mainly uses HW and demo code while using AI to modify laplacian for this use case
...
def gaussian_smooth_gray_demo56(image_gray: np.ndarray) -> np.ndarray:
    ...
    # Demo5_2dFilters-6 / Demo6 — discrete Gaussian + filter2D
...
def sobel_gradient_magnitude_demo56(image_gray: np.ndarray) -> np.ndarray:
    ...
    # Demo5_2dFilters-6 / Demo6 — Sobel + filter2D
    sobel_vert = np.array(
        [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]],
        dtype=np.float32,
    )
...
def laplacian_binary_edges(
    ...
    return (lap_mag >= t).astype(np.uint8) * 255


def laplacian_scalar_features(lap_mag: np.ndarray) -> np.ndarray:
    # Not HW — stats on |Laplacian| for RF
    flat = lap_mag.ravel()
...
def build_edge_feature_vector(
    ...
    *,
    ...
    include_sobel: bool,
) -> tuple[np.ndarray, EdgeArtifacts]:
    ...
    grad_mag = sobel_gradient_magnitude_demo56(gray_s) if include_sobel else None
    ...
    if include_sobel and grad_mag is not None:
